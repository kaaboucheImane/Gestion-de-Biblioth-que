...
if (!empty($_POST["excel"])) {
 ...
 if(in_array($extension, $allowed_extension)) //check selected file extension is present in allowed extension array
 {
  ...
  $output .= "<label class='text-success'>Données insérées</label><br /><table class='table table-bordered'>";
   $query1 = "DELETE  FROM cadreadministratif ";
  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
  {
   ...
   for($row=2; $row<=$highestRow; $row++)
   {
    ...
    $code_som=mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(3, $row)->getValue());
    ...
    $motdepasse=mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(6, $row)->getValue());
     ...
     $query = "INSERT INTO cadreadministratif(nom,prenom,code_som,cin,email,motdepasse) VALUES('".$nom."','".$prenom."','".$code_som."','".$cin."','".$email."','".$motdepasse."')";
