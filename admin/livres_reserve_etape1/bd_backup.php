<?php
     ...
     $bdd = new PDO('mysql:host=localhost;dbname=ouvrages', 'root', '',array (PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES \'UTF8\''));   
      ...
      $req2=$bdd->query("SELECT * FROM db_restaurer   ");
      while($row=$req2->fetch()){
        $x = new DateTime($row['date_restaure']);
        $x1=$x->format('Y-m-d H:i:s');
        $nb_days=$row['day'];
        $res = date('Y-m-d H:i:s',strtotime('+'.$nb_days.'days',strtotime($x1)));
...
define("DB_USER", 'root');
define("DB_PASSWORD", '');
define("DB_NAME", 'ouvrage');
define("DB_HOST", 'localhost');
define("BACKUP_DIR",'C:\wamp64\www\Projet_bibliotheque\admin\livres_reserve_etape1\db'); // Comment this line to use same script's directory ('.')
 ...
 */
     ...
     $bdd = new PDO('mysql:host=localhost;dbname=ouvrages', 'root', '',array (PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES \'UTF8\''));   
      ...
      $req2=$bdd->prepare("UPDATE  db_restaurer SET date_restaure=:d");
      $req2->execute(array(':d'=>$date1));            
...
<?php
  $bdd = new PDO('mysql:host=localhost;dbname=ouvrages', 'root', '',array (PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES \'UTF8\''));   
     ...
     // require '../../PHPMailerAutoload.php';
      ...
      $mail->isSMTP();
      while($row=$req2->fetch()){                                      // Set mailer to use SMTP
      ...
      $mail->AddAttachment("db/myphp-backup-ouvrage.sql.gz", "myphp-backup-ouvrage.sql.gz");
  }
      if(!$mail->send()) {
