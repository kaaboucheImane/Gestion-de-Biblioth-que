...
session_start();
  ...
  if (isset($_SESSION['cat']) && !empty($_SESSION['cat']) && isset($_SESSION['sous_cat']) && !empty($_SESSION['sous_cat'])) {
     $con=mysqli_connect("localhost","root","","ouvrages");
     $cat=mysqli_real_escape_string($con,$_SESSION['cat']);
     $sous_cat=mysqli_real_escape_string($con,$_SESSION['sous_cat']);
     ini_set('memory_limit', '1024M');


     
     $re=$bdd->query("SELECT * FROM categories,sous_categories,livre where livre.categorie_id=categories.id and livre.sous_categorie_id=sous_categories.id and  categorie_id like '$cat' and sous_categorie_id like '$sous_cat'  " );
     $count=$re->rowCount();
     $links=2;
     ...
     $page=$page-1;
     $p=$page*$rowsperpage;
     $req=$bdd->query("SELECT * FROM categories,sous_categories,livre where livre.categorie_id=categories.id and livre.sous_categorie_id=sous_categories.id and  categorie_id like '$cat' and sous_categorie_id like '$sous_cat'  LIMIT ".$p.",".$rowsperpage );
