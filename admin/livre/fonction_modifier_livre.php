...
<?php 
           ...
           // echo $numero.$id.$auteur.$titre.$edition.$lieu.$annee.$cote.$matiere.$nombreDeCopie.$niveau."".$n.$mois.$nb_edition.$nb_partie.$categorie.$sous_categorie." ".$collection;

            $bdd = new PDO('mysql:host=localhost;dbname=ouvrages', 'root', '',array (PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES \'UTF8\''));  
            $r=$bdd->query("UPDATE livre SET numero='$numero' , auteur='$auteur' , titre='$titre' , edition='$edition' , lieu='$lieu' , annee='$annee' , cote='$cote' , matiere='$matiere' , nombreDeCopie='$nombreDeCopie' , categorie_id='$categorie' , sous_categorie_id='$sous_categorie' , niveau='$niveau' , collection='$collection' , n='$n' , mois='$mois' , nb_partie='$nb_partie' , nb_edition='$nb_edition' WHERE id='$id' "); 
